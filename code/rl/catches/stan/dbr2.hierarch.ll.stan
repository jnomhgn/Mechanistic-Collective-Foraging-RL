...
parameters{
  ...
  real logit_alphaDBR[MAXIMUM, RATIO]; // Social learning weight for reward-based decision biasing per environment
...
model{
  ...
  real idalphaDBR; // individual-level social learning rate
  ...
  for (observation in 1:OBSERVATIONS){
      ...
      if(time[observation] != 0){ // No social info at first time step
        ...
        idalphaDBR = inv_logit(logit_alphaDBR[maximum[observation], ratio[observation]] + idoffset[id[observation], 5]);
...
generated quantities{
  ...
  real<lower = 0, upper = 1> alphaDBR[MAXIMUM, RATIO];
  matrix<lower = 0, upper = 1>[MAXIMUM, RATIO] idalphaDBR[ID]; // A vector in which each element contains (a matrix of) the learning rates of that individual
  ...
  for (observation in 1:OBSERVATIONS){
      ...
      if(time[observation] != 0){ // No social info at first time step
        ...
        p = p + idalphaDBR[id[observation], maximum[observation], ratio[observation]] * (psoc - p);
