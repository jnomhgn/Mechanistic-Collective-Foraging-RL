...
model {
  ...
  for (observation in 1 : OBSERVATIONS) {
    if (time[observation] == 0) {
      ...
      idalphaVSDR = inv_logit(logit_alphaVSDR[maximum[observation], ratio[observation]]
      ...
      if (obsrew[observation] == 100) {
        ...
        Qsoc = (1 - idsigmaVSDR)
               * obsdec[observation, decision[observation - 1]]
               + idsigmaVSDR * obsrew[observation];
      }
...
generated quantities {
  ...
  array[ID] matrix[MAXIMUM, RATIO] idalphaVSDR;
  real<lower=0, upper=1> sigmaVSDR;
  array[ID] real<lower=0, upper=1> idsigmaVSDR;
  for (i in 1 : ID) {
    ...
    for (m in 1 : MAXIMUM) {
      for (r in 1 : RATIO) {
        idalphaVSDR[i, m, r] = inv_logit(logit_alphaVSDR[m, r]
                                         + idoffset[i, 5]);
      }
    }
  }
  ...
  betaQ = mean(idbetaQ);
  sigmaVSDR = mean(idsigmaVSDR);
  for (m in 1 : MAXIMUM) {
    for (r in 1 : RATIO) {
      real accumulatedVSDR = 0;
      for (i in 1 : ID) {
        accumulatedVSDR += idalphaVSDR[i, m, r];
      }
      alphaVSDR[m, r] = accumulatedVSDR / ID;
    }
  }
  ...
  for (observation in 1 : OBSERVATIONS) {
    if (time[observation] == 0) {
      ...
      if (obsrew[observation] == 100) {
        ...
        Qsoc = (1 - idsigmaVSDR[id[observation]])
               * obsdec[observation, decision[observation - 1]]
               + idsigmaVSDR[id[observation]] * obsrew[observation];
      }
      Q[decision[observation - 1]] = Q[decision[observation - 1]]
                                     + idalphaVSDR[id[observation], maximum[observation], ratio[observation]]
                                       * (Qsoc - Q[decision[observation - 1]]);
    }
