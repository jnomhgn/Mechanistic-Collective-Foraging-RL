...
parameters{
  ...
  real logit_alphaVSDR[MAXIMUM, RATIO]; // Social learning weight for decision- and reward-based value shaping per environment
  real logit_sigmaVSDR; // Relative influence of observed decisions and rewards on social value fixed across environments
...
model{
  ...
  real idalphaVSDR; // individual-level social learning rate
  real idsigmaVSDR; // individual-level relative influence of rewards and decisions
  ...
  for (observation in 1:OBSERVATIONS){
      ...
      if(time[observation] == 0){
        ...
        idalphaVSDR = inv_logit(logit_alphaVSDR[maximum[observation], ratio[observation]] + idoffset[id[observation], 5]);
...
generated quantities{
  ...
  real<lower = 0, upper = 1> alphaVSDR[MAXIMUM, RATIO];
  matrix[MAXIMUM, RATIO] idalphaVSDR[ID]; // A vector in which each element contains (a matrix of) the learning rates of that individual
  
  real<lower = 0, upper = 1> sigmaVSDR;
  real<lower=0, upper = 1> idsigmaVSDR[ID];
  ...
  for(i in 1:ID){
    ...
    idsigmaVSDR[i] = inv_logit(logit_sigmaVSDR + idoffset[i, 6]);

    for(m in 1:MAXIMUM){
      for(r in 1:RATIO){
        idalphaVSDR[i, m, r] = inv_logit(logit_alphaVSDR[m, r] + idoffset[i, 5]);
      }
    }
  ...
  for (observation in 1:OBSERVATIONS){
	...
	if(obsrew[observation] == 100){ // If nobody else at patch of who to observe rewards
        ...
        // Shape values
        Q[decision[observation-1]] = Q[decision[observation-1]] + idalphaVSDR[id[observation], maximum[observation], ratio[observation]] * (Qsoc - Q[decision[observation-1]]);
