...
model {
  ...
  for (observation in 1 : OBSERVATIONS) {
    if (time[observation] == 0) {
      ...
      idalphaVSR = inv_logit(logit_alphaVSR[maximum[observation], ratio[observation]]
...
generated quantities {
  ...
  array[ID] matrix[MAXIMUM, RATIO] idalphaVSR;
  for (i in 1 : ID) {
  ...
  for (m in 1 : MAXIMUM) {
    for (r in 1 : RATIO) {
      real accumulatedVSR = 0;
      for (i in 1 : ID) {
        accumulatedVSR += idalphaVSR[i, m, r];
      }
      alphaVSR[m, r] = accumulatedVSR / ID;
    }
  }
  ...
  for (observation in 1 : OBSERVATIONS) {
    if (time[observation] == 0) {
      ...
      if (Qsoc != 100) {
        Q[decision[observation - 1]] = Q[decision[observation - 1]]
                                       + idalphaVSR[id[observation], maximum[observation], ratio[observation]]
                                         * (Qsoc
                                            - Q[decision[observation - 1]]);
      }
    }
