...
parameters{
  ...
  real logit_alphaDBR[MAXIMUM, RATIO]; // Social learning weight for reward-based decision biasing
...
model{
  ...
  for (observation in 1:OBSERVATIONS){
      ...
      if(time[observation] != 0){ // No social info at first time step
        ...
        idalphaDBR = inv_logit(logit_alphaDBR[maximum[observation], ratio[observation]] + idoffset[id[observation], 6]);
