...
nsim = get_pipeline_value("rl", "nocatches", "parrecov", "nsim", default = 10)
chains = get_pipeline_value("rl", "nocatches", "parrecov", "chains", default = 4)
cores = get_pipeline_value("rl", "nocatches", "parrecov", "cores", default = 4)
iter = get_pipeline_value("rl", "nocatches", "parrecov", "iter", default = 2000)
warmup = get_pipeline_value("rl", "nocatches", "parrecov", "warmup", default = 1000)
refresh = get_pipeline_value("rl", "nocatches", "parrecov", "refresh", default = 100)
exp_sessions = get_pipeline_value("rl", "nocatches", "parrecov", "sessions", default = 18)
exp_trials = get_pipeline_value("rl", "nocatches", "parrecov", "trials", default = 12)
exp_nplayers = get_pipeline_value("rl", "nocatches", "parrecov", "nplayers", default = 5)
exp_durations = get_pipeline_value("rl", "nocatches", "parrecov", "durations_vec", default = c(75, 90, 105))
...
resultsdir <- file.path("results", "rl", "nocatches", "parrecov")
...
exp.pars = list(
  ...
  trials = exp_trials,
  nplayers = exp_nplayers,
  durations.vec = exp_durations
)
...
# Load winning model
load(file = file.path("results", "rl", "nocatches", "modelcomp", "adaptive", "modelcomp.Rdata"))
...
for(mod in 1:length(models$name)){
  ...
  if(!file.exists(file.path(resultsdir, paste(models$name[[mod]], "rds", sep = ".")))){
    ...
    results = bind_rows(results) %>%
      mutate(par = factor(par, levels = names(models$free.pars.pop[[mod]])))
