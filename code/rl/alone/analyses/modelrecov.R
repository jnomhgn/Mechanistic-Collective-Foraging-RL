...
if(!dir.exists(file.path("results", "rl", "alone", "modelrecov", "diagnostics", "detailed"))){dir.create(file.path("results", "rl", "alone", "modelrecov", "diagnostics", "detailed"), recursive = T)}

resultsdir <- file.path("results", "rl", "alone", "modelrecov")
...
computeloo <- function(msim, sim, models, stan.data){
  ...
  # Save Variables
  saveRDS(list(results = results, comparison = comparison, winner = winner), file = file.path(resultsdir, paste(models$name[[msim]], sim, "rds", sep = ".")))

  # Save and print comparison
  write.csv(x = comparison, file = file.path(resultsdir, paste(models$name[[msim]], sim, "csv", sep = ".")))
