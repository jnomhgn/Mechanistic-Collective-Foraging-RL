...
m1.1.fixed.sim <- function(sim.parameters){
  ...
  with(data = sim.parameters, expr = {
    ...
    for(session in 1:sessions){
      ...
      # Permutate durations for each session (so that counterbalanced across trials)
      ...
      for(trial in 1:trials){
        ...
        for(time in 0:durations[trial]){
          ...
          for(player in 1:nplayers){
            ...
            Q[player, decision] = Q[player, decision] + alphaQ * (reward - Q[player, decision])
