...
parameters{
  ...
  real logit_alphaQN; // Different asocial learning weights for negative reward prediction errors between levels of resource abundance
  real logit_alphaQP; // Different asocial learning weights for position rpes between levels of resource abundance
  ...
  // adds scaling.
  matrix[3, ID] z;           // z - values
  vector<lower=0>[3] sigma;       // SDs
  cholesky_factor_corr[3] cholesky;   // Cholesky factor matrix
  
}

transformed parameters{
  matrix[ID, 3] idoffset; // Matrix of varying effects for each individual
...
generated quantities{
  ...
  // Declare variables
  matrix[3, 3] Rho; // Correlation matrix
