...
parameters {
  array[MAXIMUM] real logit_alphaQN;
  array[MAXIMUM] real logit_alphaQP;
  real log_betaQ;
  real betaC;
  matrix[4, ID] z;
...
model {
  ...
  for (observation in 1 : OBSERVATIONS) {
    ...
    if ((reward[observation] - Q[decision[observation]]) < 0) {
      idalphaQ = inv_logit(logit_alphaQN[maximum[observation]]
                           + idoffset[id[observation], 1]);
    }
    else {
      idalphaQ = inv_logit(logit_alphaQP[maximum[observation]]
...
generated quantities {
  matrix[4, 4] Rho;
  array[MAXIMUM] real<lower=0, upper=1> alphaQN;
  array[ID, MAXIMUM] real<lower=0, upper=1> idalphaQN;
  array[MAXIMUM] real<lower=0, upper=1> alphaQP;
  array[ID, MAXIMUM] real<lower=0, upper=1> idalphaQP;
  real<lower=0> betaQ;
  ...
  for (m in 1 : MAXIMUM) {
    ...
    for (i in 1 : ID) {
      accumulatedQN += idalphaQN[i, m];
      accumulatedQP += idalphaQP[i, m];
    }
    alphaQN[m] = accumulatedQN / ID;
    alphaQP[m] = accumulatedQP / ID;
  }
  ...
  for (observation in 1 : OBSERVATIONS) {
    ...
    if ((reward[observation] - Q[decision[observation]]) < 0) {
      Q[decision[observation]] = Q[decision[observation]]
                                 + idalphaQN[id[observation], maximum[observation]]
                                   * (reward[observation]
                                      - Q[decision[observation]]);
    }
    else {
      Q[decision[observation]] = Q[decision[observation]]
                                 + idalphaQP[id[observation], maximum[observation]]
                                   * (reward[observation]
                                      - Q[decision[observation]]);
    }
    C = [0, 0]';
