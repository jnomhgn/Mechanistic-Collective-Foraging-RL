...
for icond in range(3):
        for ip1 in range(len(Lp1)):
                ...
                for iratio in range(len(Lratio)):
                        ...
                        if iratio<3:
                                ax[iratio,ip1].tick_params(bottom=False,labelbottom=False)
...
for icond in range(1, 3):
        ...
        if Lc[icond]==2:
                fig.supxlabel(r'Social Learning Weight')# $\alpha_{DB,l/VS,l}$')
        elif Lc[icond]==3:
                fig.supxlabel(r'Social Learning Weight')# $\alpha_{DB,r/VS,r}$')
        ...
        for ic in [0,1]:
                ...
                for ip1 in range(len(Lp1)):
                        p1 = Lp1[ip1]
                        ax[0,ip1].set_title('Max catch' + str(p1),fontsize=10)
                        indp = np.where(df['max']==p1)[0]
                        ...
                        for iratio in range(len(Lratio)):
                                ...
                                ax[iratio,-1].yaxis.set_label_position("right")
                                ax[iratio,-1].set_ylabel('Catch Ratio' + str(ratio),fontsize=10)
                                ...
                                if p1>0.5:
                                        ax[iratio,ip1].tick_params(left = False)
                                if iratio < 3:
                                        ax[iratio,ip1].tick_params(bottom = False)
                                ax[iratio,ip1].set_xticks([0,0.5,1.0])
                                ax[iratio,ip1].set_xticklabels(['0','0.5','1.0'], fontsize=10)
                                ...
                                if Lc[icond]==2:
                                        ...
                                        ax[iratio,ip1].set_yticklabels(['-0.5','-0.25','0','0.25'], fontsize=10)
                                else:
                                        ax[iratio,ip1].set_ylim(-0.4,0.2)
                                        ax[iratio,ip1].set_yticks([-0.3,-0.15,0,0.15])
                                        ax[iratio,ip1].set_yticklabels(['-0.3','-0.15','0','0.15'], fontsize=10)
