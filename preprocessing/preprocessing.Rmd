...
for (file in files) {
  ...
  trial_list <- read.csv2(paste0("sessions/session",session,".csv"))
  
  trials <- list.files(path = paste0("data/",file), pattern = "trial")
  ...
  for (trial in trials) {
    ...
    dat_new <- read.csv2(paste0("data/", file,"/",trial)  )
...
# Save in wide format
write.csv(dat, file = 'data_wide.csv')
```
...
# Save
write.csv(dat, file = 'data_long.csv')
...
write.csv(dat.discrete, file = 'data_discrete_1s.csv')
```
...
# Save
write.csv(x = dems.total, file = "demographics.csv")
...
# Pay
d = read.csv("~/Users/MarienhagenJonathan/thesisvc/data/data_long.csv") %>% 
