...
LQexp = np.load('./files/LQexp1s'+str(Lc[0])+'.npy')
MQexp = np.load('./files/MQexp1s'+str(Lc[0])+'.npy')
MQs = np.zeros((len(Lp1),len(Lratio),18,75))
for ns in range(18):
        MQs[:,:,ns,:] = np.mean(MQexp[:,:,ns*5:5+5*ns,:],axis=2)
...
for M in range(len(Lp1)):
        for R in range(len(Lratio)):
                ...
                if R == 3:
                        ...
                        ax[R].plot(lt1s,MaccS[M,R,0:75],color=col[M])
...
fig.savefig('./figures/AccFit'+str(co)+'.pdf',bbox_inches='tight')
