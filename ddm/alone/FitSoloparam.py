...
#[alpha,B,yM,tauy]
vecPmin = [0,0,0,1]
vecPmax = [5, 2.5, 10,100]


LQ = np.load('./files/LQexp1s1.npy')
LQin = np.load('./files/LQinexp1.npy')
MQ = np.load('./files/MQexp1s1.npy')
        ...
        def fitness(self,vecP):
                ...
                for ip1 in range(len(Lp1)):
                        ...
                        for iratio in range(len(Lratio)):
                                ...
                                [LavgS,LaccS,MaccS,LrateS,LX,Lrw,T0,Ly] = Simu(alpha,theta,B,yM,tauy,p0,p1,iDeltar,N,co,kr,0,0,eta,idrate,Ttr,Lt,dt,nsimu,Qin)
...
algo = pg.algorithm(pg.cmaes(gen=40,force_bounds = True))
algo.set_verbosity(1)
pop = pg.population(prob,40)
print(pop.champion_f)
print('alpha,theta,B,yM,tauy',pop.champion_x)
np.save('./files/VecP',pop.champion_x)
np.save('./files/fitP',pop.champion_f)
