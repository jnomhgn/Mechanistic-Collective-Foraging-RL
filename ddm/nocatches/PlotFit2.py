...
co = 2 # 1 DB, 2 VS (VS win)
Lc = [2]

if co==1:
        # DB, co=1
        VecP2 = [1.95119819, 0.86123321, 0.10161255, 6.84979781, 3.39912024, 0.0414931 ]
        fitP2 =  0.030056045781646095
        #np.save('./files/VecP2'+str(co),VecP2)
        #np.save('./files/fitP2'+str(co),fitP2)

elif co==2:
        # VS, co=2
        VecP2 = [1.18383224, 0.46982191, 0.38107954, 5.08829568, 0.50315738, 0.60573084]
        fitP2 =  0.027965347493580255 
        #np.save('./files/VecP2'+str(co),VecP2)
        #np.save('./files/fitP2'+str(co),fitP2)
...
np.save('./files/Meanddm2All',Mean)					
print('simu done')  
...
for M in range(len(Lp1)):
        for R in range(len(Lratio)):
                ...
                if R == 3:
                        ...
                        ax[R].plot(lt1s,MaccS[M,R,0:75],color=col[M])#,linestyle='--')
