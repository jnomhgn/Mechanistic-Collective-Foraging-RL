...
# threshold fixed
theta = -5
#[alpha,B,yM,tauy,kd,eta]
vecPmin = [0,0,0,1,0,0]
vecPmax = [5, 2.5, 10,50,10,1]

LQ = np.load('./files/LQexp1s2.npy')
LQin = np.load('./files/LQinexp2.npy')
MQ = np.load('./files/MQexp1s2.npy')
...
algo = pg.algorithm(pg.cmaes(gen=60,force_bounds = True))
algo.set_verbosity(1)
pop = pg.population(prob,50) 
print(pop.champion_f)
print('alpha,B,yM,tauy,kd,eta',pop.champion_x)

print('evolve...')
pop = algo.evolve(pop)
print(pop.champion_f)
print('alpha,B,yM,tauy,kd,eta',pop.champion_x)
np.save('./files/VecP2'+str(co),pop.champion_x)
np.save('./files/fitP2'+str(co),pop.champion_f)
