...
if(!file.exists(paste("social/results/modelcomp_dec/full_adaptive", "modelcomp_dec_full_adaptive.Rdata", sep = "/"))){
  ...
  for(mfit in 1:length(models$stan.loglik)){
    ...
    # Plot detailed traceplots
    if(!dir.exists(
      paste("social/results/modelcomp_dec/full_adaptive/diagnostics/detailed",models$name[[mfit]] , sep = "/"))){
      dir.create(paste("social/results/modelcomp_dec/full_adaptive/diagnostics/detailed",models$name[[mfit]] , sep = "/"))
      }
    draws = tidy_draws(fit)
    par.names = names(draws)
    par.names = par.names[which(! names(draws) %in% c(".chain", ".iteration",".draw", "lp__",
    ...
    for (param in par.names) {
      tplot <- traceplot(fit, pars = param) + ggtitle(paste("Trace plot for", param))
      ggsave(paste("social/results/modelcomp_dec/full_adaptive/diagnostics/detailed",models$name[[mfit]],  paste0("traceplot_", param, ".png"), sep = "/"), tplot)
    }
  ...
  # Save Variables
  save(results, comparison, winner, file = paste("social/results/modelcomp_dec/full_adaptive", "modelcomp_dec_full_adaptive.Rdata", sep = "/"))
  # Save and print comparison
...
## Posterior predictions
```{r, postpred-modelcomp_dec_full_adaptive, echo=TRUE}
...
write.csv(plot.data, file = paste("social", "results", "modelcomp_dec", "full_adaptive", "postpredict_nocatches.csv", sep = "/"))
