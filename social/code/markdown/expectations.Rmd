...
plot.data = results$rew.acc %>%
    ...
    reframe(acc.mean = mean(acc.mean)) %>%
    filter((model == "arl.fixed" & alphaS == 0) | model != "arl.fixed" ) %>%
    group_by(ratio, max) %>%
    mutate(acc.delta = acc.mean - acc.mean[which(model == "arl.fixed")])
write.csv(plot.data, file = paste("social", "results", "expectations", "expec_accdiff_lisa.csv", sep = "/"))
...
p = plot.data %>% filter(model %in% c("dbn1.fixed", "vsn1.fixed")) %>%
      ...
      geom_point(aes(y=acc.delta), shape=21, size=1) +
...
write.csv(plot.data, file = paste("social", "results", "expectations", "expec_acctimediff.csv", sep = "/"))
...
write.csv(plot.data, file = paste("social", "results", "expectations", "expec_switchtimediff.csv", sep = "/"))
