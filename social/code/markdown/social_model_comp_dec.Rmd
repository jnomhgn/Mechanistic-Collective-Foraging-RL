...
if(!dir.exists("social/results/modelcomp_dec/diagnostics")){dir.create("social/results/modelcomp_dec/diagnostics")}
```
...
d = d %>% 
  mutate(obs.dec.1 = ifelse(is.na(obs.dec.1), 100, obs.dec.1)) %>%
  mutate(obs.dec.2 = ifelse(is.na(obs.dec.2), 100, obs.dec.2))
...
stan.data.d = with(d, list(
  ...
  obsdec=cbind(obs.dec.1, obs.dec.2),
...
if(!exists(paste("social/results/modelcomp_dec", "modelcomp_dec.Rdata", sep = "/"))){
  ...
  for(mfit in 1:length(models$stan.loglik)){
    ...
    log.file = paste(paste("social/results/modelcomp_dec",
                                    "log.txt", sep = "/"))
    ...
    sink()
    saveRDS(fit, paste("social/results/modelcomp_dec", paste(models$name[[mfit]], "fit", "rds", sep = "."), sep = "/"))
    ...
    ggexport(plotlist = diag.list, width = 1920, height = 1080,
                   filename = paste("social/results/modelcomp_dec/diagnostics",
                                    paste(models$name[[mfit]], "diagnostics", "jpeg",  sep = "."), sep = "/"))

    # Save diagnostics for all parameters
    fit.summary = summary(fit)$summary
    write.csv(fit.summary, file = paste("social/results/modelcomp_dec/diagnostics",
    ...
    ggexport(p, width = 2550, height = 1440,
             filename = paste("social/results/modelcomp_dec", paste(models$name[[mfit]], "draws", "jpeg", sep = "."), sep = "/"))
    ...
    # Save diagnostics
    jpeg(paste("social/results/modelcomp_dec", paste(models$name[[mfit]], "paretok", "jpeg", sep = "."), sep = "/"))
    plot(get(loo.model))
    dev.off()
  ...
  # Save Variables
  save(results, comparison, comparison.named, winner, file = paste("social/results/modelcomp_dec", "modelcomp_dec.Rdata", sep = "/"))
  
  # Save and print comparison
  write.csv(x = comparison, file = paste("social/results/modelcomp_dec", "modelcomp_dec.csv", sep = "/"))
  write.csv(x = comparison.named, file = paste("social/results/modelcomp_dec", "modelcomp_dec_named.csv", sep = "/"))
